...
export function LoginPage() {
  ...
  const { user, ready, ssoConfigured, ssoGaps, allowedDomains } = useSession();

  const from = useMemo(() => safeFrom(searchParams.get('from') ?? undefined), [searchParams]);
  const domains = allowedDomains?.length ? allowedDomains : ['spiralyze.com'];

  const errorParam = searchParams.get('error') ?? '';
  const errorMessage = errorParam ? ERROR_MESSAGES[errorParam] ?? 'Sign-in failed. Please try again.' : null;
  const showSignedOut = searchParams.get('signedOut') === '1' && !errorParam;
  ...
  return (
    <div className="mx-auto flex min-h-[70vh] max-w-md flex-col items-stretch justify-center gap-6 py-10">
      <div className="text-center">
        <div className="text-[11.5px] font-semibold uppercase tracking-[0.18em] text-text-muted">{BRAND.name}</div>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-text">Sign in</h1>
        <p className="mt-1 text-[13px] text-text-muted">
          {domains.length === 1
            ? `Use your @${domains[0]} Google Workspace account.`
            : `Use a Google account in: ${domains.map(d => `@${d}`).join(', ')}.`}
        </p>
      </div>
      ...
      {!ssoConfigured ? (
        <div className="rounded-lg border border-status-yellow/30 bg-status-yellow/5 p-4 text-[12.5px] leading-relaxed text-status-yellow">
          ...
          <div className="mt-1 text-text-dim">
            Set <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px]">GOOGLE_CLIENT_ID</code>,{' '}
            <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px]">GOOGLE_CLIENT_SECRET</code>,{' '}
            <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px]">NEXTAUTH_SECRET</code>, and{' '}
            <code className="rounded bg-surface-2 px-1.5 py-0.5 font-mono text-[11px]">NEXTAUTH_URL</code> on the
            API server, then reload.
            {ssoGaps.length ? (
              <span className="mt-2 block text-[11px] text-text-muted">Missing: {ssoGaps.join(', ')}</span>
            ) : null}
          </div>
        </div>
      ) : (
        <div className="flex flex-col gap-4 rounded-xl border border-border bg-surface/60 p-5 shadow-card-hover">
              ...
              {errorParam === 'domain_not_allowed' && (
                <div className="mt-1 text-text-dim">
                  {searchParams.get('email') ? (
                    <>
                      <span className="font-mono">{searchParams.get('email')}</span> is not in the allow-list.
                    </>
                  ) : null}
                  {searchParams.get('allowed') && (
                    <div className="mt-0.5">
                      Allowed domains: <span className="font-mono">{searchParams.get('allowed')}</span>
                    </div>
                  )}
                </div>
              )}
          ...
          <a
            ...
            className="inline-flex h-10 items-center justify-center gap-2.5 rounded-md border border-border bg-white px-4 text-[13px] font-medium text-[#1f1f1f] transition hover:border-border-strong hover:bg-[#f6f8fc]">
